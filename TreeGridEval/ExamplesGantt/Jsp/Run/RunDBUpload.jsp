   ...
   try {
      ...
      for(int i=0;i<Ch.getLength();i++){
         ...
         if(I.getAttribute("Deleted").equals("1")){ // Deleting
	         Cmd.executeUpdate("DELETE FROM Run WHERE ID="+id);  
            }
         else if(I.getAttribute("Added").equals("1")){ // Adding
            Cmd.executeUpdate("INSERT INTO Run(ID,T,S,R) VALUES("
               + "'" + id + "',"
               + "'" + I.getAttribute("T").replaceAll("'","''") + "'," 
               + "'" + I.getAttribute("S") + "'," 
               + "'" + I.getAttribute("R").replaceAll("'","''") + "')");
            }
         else if(I.getAttribute("Changed").equals("1")){ // Updating
				StringBuffer SQL = new StringBuffer();
			 	org.w3c.dom.Node N;
				SQL.append("UPDATE Run SET ");
				...
				N = I.getAttributeNode("R"); if(N!=null) SQL.append("R='"+N.getNodeValue().replaceAll("'","''")+"',");
