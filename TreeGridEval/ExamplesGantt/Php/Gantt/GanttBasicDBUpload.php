...
if ($XML) { 
   ...
   foreach($AI as $I){
      ...
      if(!empty($A["Deleted"])){
         $db->Exec("DELETE FROM GanttBasic WHERE id=".$A["id"]);
         }
      else if(!empty($A["Added"])){
         $db->Exec("INSERT INTO GanttBasic(id,T,S,E,C,D) VALUES("
               . $A["id"] . "," 
               . "'" . str_replace("'","''",$A["T"]) . "',"
               . "'" . $A["S"] . "',"
               . "'" . $A["E"] . "',"
         ...
         $S = "UPDATE GanttBasic SET ";
         foreach($A as $name => $value){
            if($name=="T") $S .= "$name = '" . str_replace("'","''",$value) . "',";
            else if($name=="S" || $name=="E" || $name=="C" || $name=="D") $S .= "$name = '$value',";
            }
