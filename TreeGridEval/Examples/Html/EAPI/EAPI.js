...
var Path = document.getElementById("MenuAll") ? "../" : "../../../";  // Path to root folder, it is different when shown on TreeGrid homepage
...
EAPIOnLoad = window.onload;

window.onload = function(ev){
if(EAPIOnLoad) EAPIOnLoad(ev);

// --- Prepares and clears controls on pages ---
DLog = document.getElementById("LOG");
IStyle = document.getElementById("IStyle");
IDisable = document.getElementById("IDisable");
DLog.value = "";
IDisable.checked = false;
IStyle.checked = false;
document.getElementById("IColor").value = "hdd";
document.getElementById("SData").selectedIndex = 0;
if(!window.IFunc) IFunc = document.getElementById("IFunc");
if(!window.IStyle) IStyle = document.getElementById("IStyle");
if(!window.IDisable) IDisable = document.getElementById("IFunc");

// --- creates grid ---
var D = {Layout:{},Data:{},Defaults:{},Upload:{}};   // new object for communication
...
D.Defaults.Bonus="<Grid><LeftCols><C Name='id' CanEdit='0'/></LeftCols><Header id='id'/></Grid>"; // To every example adds column with row ids
D.Layout.Bonus="<Grid><Cfg MinTagHeight='450' LimitScroll='0'/></Grid>";
D.Upload.Url = "dummy";
D.Debug = "check";
...
function LoadData(){
var idx = document.getElementById("SData").selectedIndex; // The first combo, contains selected data
...
Grids.OnGetColor = function(G,row,col,r,g,b,type){ 
   ...
   if(G.id=="Books" && !Get(row,"Spanned") && !row.Fixed) return "rgb("+r+","+g+","+(b-20)+")";// Support function for particular example
}
...
Grids.OnTabOutside = function(G,move){
   if(move>0){
      ...
      document.getElementById("IStyle").focus();
      return true;
      }   
}
...
// --------------------------------------------------------------------------------------------------------------------------
// Catches debug print
Grids.OnDebug = function(G,level,items){ 
if(level>=3) return true; // Ignores informational messages here
for(var i=1,s="";i<items.length;i++) s += i%2 ? items[i] : "<b>"+items[i]+"</b>"; // Cannot use join, items is not an Array
Log("OnDebug: "+["Fatal error","Error","Warning","List","Info"][level]+" "+s,1,["red","red","orange","silver","gray"][level]); 
return true;
}
...
var S = document.getElementById('SIds');
var id = S.options[S.selectedIndex].value;
var G = Grids[0];
var row = G.GetRowById(id);
var S = document.getElementById('SCols');
...
setTimeout(function(){ // Delays the code, otherwise the grid will loose focus to the select box
   G.Focus(row,col,null,null,1);   // Focuses the row and also expands row's parent if it is collapsed (set by last parameter)
   },10);
}
// --------------------------------------------------------------------------------------------------------------------------
...
var D = document.getElementById("DRow"), A = new Array(), p=0;
A[p++] = "<div id=ID_ID>Row attributes:</div>";
...
var G = Grids[0];
var V = document.getElementById("IColor").value;
