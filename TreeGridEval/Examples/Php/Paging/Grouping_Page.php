...
for($i = $Level==0?$Start:0; $i<$cnt;$i++){
   ...
   if ($Level == $Levels) {    // Final, writing data row
      ...
      WriteLevel($Level + 1, $Where . $Where2, $GroupCols, $db, $Levels, 0);
      echo "</I>";
      }
   }
}
...
if(!$XML) $XML = "<Grid><Cfg GroupCols='Resource,Week'/><Body><B Pos='2'/></Body></Grid>"; // Just for examples if called directly
...
echo "<Grid><Body><B Pos='" . $B["Pos"] . "'>"; // XML Header
WriteLevel(0, "", $GC, $db, $cnt, $start);
