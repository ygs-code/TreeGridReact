...
<html>
   ...
   <body>
      ...
      <center class="ExampleName">Page in old ASP style</center>
      <center class="ExampleShort">Uses old style page creating by direct code instead of use Page_Load event, uses <b>&lt;form> submit</b> and <b>TreeGrid ASP.NET framework</b></center>
      <div class="ExampleDesc">
         <i>Source files:</i> <h4>FrameworkAspStyle.aspx</h4> (this html page and also server script that generates and processes XML data), <a href="DBDef.xml" target="_blank"><h4>DBDef.xml</h4></a> (static XML layout), 
         <h4>../Database.db</h4> (source SQL database, table <b>TableData</b>)<br />
         <h4>../Framework/TreeGridFramework.aspx</h4> (TreeGrid ASP.NET framework support script, included into <b>Framework.aspx</b> script)
      </div>
      <div class="ExampleDesc">
         This example uses <h4>SQLite</h4> <b>../Database.db</b> file as source SQL database.</h4>
         You can switch to <h4>MS Access</h4> <b>MDB database</b> by uncomment the line "Data Source=..." in FrameworkAspStyle.aspx. <br />
         <u>
...
<%
   ...
   If XML <> "" And XML <> Nothing Then Grid.SaveXMLToDB(XML) ' Saves data to database
   XML = Grid.LoadXMLFromDB().Replace("'", "&apos;").Replace("&", "&amp;")  ' Loads data from database  
   ' -------------------------------------------------------------------------------------------------------------------------------
%>
      ...
      <script>
         var TGTrial = document.cookie.match(/TGTrialVB\s*=\s*(\d+)/), TGIndex = 32768;
         if(!TGTrial||!(TGTrial[1]&TGIndex)) setTimeout(function(){
            var n = "RunTrialVBFrameworkAspStyleSubmit", d = (new Date((new Date).getTime()+31536000000)).toUTCString(); document.cookie = "TGTrialVB="+((TGTrial?TGTrial[1]:0)|TGIndex)+";expires="+d;
