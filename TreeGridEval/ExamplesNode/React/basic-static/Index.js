...
class Sample extends React.Component {
   ...
   // Links this React component with the created TreeGrid. The React component will be accessible in grid events as Grid.Component and grid will be accessible from React code as this.Grid
   // The Grids.OnInit can be used only if there is only one TreeGrid on page. For more grids on page assign the event only for particular grid as: TGSetEvent("OnInit",grid_id,function(G){ G.Component = Component; Component.Grid = G; });
   Grid = null;
   constructor(){
      super();
      var Component = this;
      Grids.OnInit = function(grid){ grid.Component = Component; Component.Grid = grid; return null; }
   ...
   componentDidMount() {
      StartTreeGrid(); // Processes all newly rendered <treegrid> / <bdo> tags on page
      }
...
<div className="ExampleShort">Simple example without any installation; Creates TreeGrid statically by &lt;treegrid> tag</div>
...
<div className="ExampleBorder">
   ...
   <div className="ExampleMain" style={{width:"100%",height:"530px"}}>

      {/* TreeGrid definition */}
      {/* Displays the first example on load */}
      {/* The is attribute is used to suppress React16+ warning about custom tag */}
      {/* By the id value the grid is be accessible for global API like Grids.SampleGrid.Reload(); */}
